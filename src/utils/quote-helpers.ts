import { isError, isObject, has } from "lodash";
import { QuoteError, QUOTE_ERROR_KEYS } from "@/types/quote";
...
export const createHttpError = (status: number, message: string): QuoteError =>
  createQuoteError(QUOTE_ERROR_KEYS.HTTP_ERROR, { status, message });

export const createFetchError = (error: unknown): QuoteError => {
  if (isError(error)) {
    return createQuoteError(QUOTE_ERROR_KEYS.FETCH_FAILED, { message: error.message });
  }
